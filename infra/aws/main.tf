...
resource "aws_security_group" "api" {
  ...
  tags = merge(local.common_tags, { Name = "${local.name}-api" })
}

resource "aws_security_group" "indexer" {
  name        = "${local.name}-indexer"
  description = "Indexer ECS tasks"
  vpc_id      = aws_vpc.main.id
  ...
  tags = merge(local.common_tags, { Name = "${local.name}-indexer" })
}
...
resource "aws_ecs_task_definition" "indexer" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "ATTESTATION_REGISTRY_ADDRESS", value = var.attestation_registry_address },
      { name = "SOLVER_REGISTRY_ADDRESS", value = var.solver_registry_address },
      { name = "ATTESTOR_REGISTRY_ADDRESS", value = var.attestor_registry_address },
      { name = "COMMERCE_REGISTRY_ADDRESS", value = var.commerce_registry_address },
      { name = "POLL_INTERVAL_MS", value = "2000" },
      { name = "LOG_LEVEL", value = "info" }
    ]
    secrets = [
      { name = "DATABASE_URL", valueFrom = "${aws_secretsmanager_secret.app.arn}:DATABASE_URL::" }
...
resource "aws_cloudfront_function" "frontend_rewrite" {
  ...
  comment = "Rewrite extensionless static export routes to index.html"
  publish = true
  code    = <<EOF
